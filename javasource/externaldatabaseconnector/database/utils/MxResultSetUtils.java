...
import externaldatabaseconnector.database.exceptions.MxInvalidNameException;
...
public class MxResultSetUtils {
  ...
  public static TableResult buildTableResult(ResultSet resultSet) throws MxDataTypeNotSupported, SQLException, MxInvalidNameException {
    ...
    for (int i = 1; i <= columnCount; i++) {
      ...
      //validateColumnName(columnName, whiteSpacePattern);
  ...
  private static void validateColumnName(String columnName, Pattern whiteSpacePattern) throws MxInvalidNameException {
    if (columnName.isEmpty() || whiteSpacePattern.matcher(columnName).find()) {
      String exceptionMessage = String.format(IMxErrorMessages.INVALID_COLUMN_NAME, columnName);
      String messageCode = "E070";
      Map<String, String> messageDataMap = new HashMap<>();
      messageDataMap.put("columnName", columnName);
      throw new MxInvalidNameException(exceptionMessage, messageCode, messageDataMap);
    }
  }
